...
Los grupos de almacenamiento en la nube también requieren que los nodos de almacenamiento tengan acceso a los endpoints proporcionados por el servicio externo utilizado, como Amazon S3 Glacier o Microsoft Azure Blob Storage. Para obtener más información, consulta link:../ilm/what-cloud-storage-pool-is.html["Qué es un pool de almacenamiento en la nube"].
