...
summary: La API de REST de S3 que utiliza StorageGRID se actualiza periódicamente. 
---
= Cambios en la compatibilidad con la API de REST de StorageGRID S3
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
La API de REST de S3 que utiliza StorageGRID se actualiza periódicamente.
...
* Se añadió compatibilidad con valores de suma de comprobación precalculados para las siguientes solicitudes y encabezados compatibles:
+
** CompleteMultipartUpload: `x-amz-checksum-crc32`, `x-amz-checksum-crc32c` y `x-amz-checksum-crc64nvme`
** CreateMultipartUpload: `CRC32`, `CRC32C` y `CRC64NVME` para `x-amz-checksum-algorithm`
** CopyObject: `CRC32`, `CRC32C`, `CRC64NVME` y `SHA256` para `x-amz-checksum-algorithm`
** PutObject: `x-amz-checksum-crc32`, `x-amz-checksum-crc32c` y `x-amz-checksum-crc64nvme`
** UploadPart: `x-amz-checksum-crc32`, `x-amz-checksum-crc32c` y `x-amz-checksum-crc64nvme`


* Se ha añadido compatibilidad con `x-amz-checksum-type` para CreateMultipartUpload y CompleteMultipartUpload.
* Se añadió compatibilidad para la clave de condición de política  `s3:x-amz-copy-source`.
* Permitir que se active S3 Object Lock en los buckets existentes.
...
* Se ha añadido compatibilidad con STS AssumeRole y la política de sesión. Consulta link:../s3/example-session-policies.html["un ejemplo de una política de sesión"]. Puedes configurar AssumeRole en grupos de inquilinos.
...
11,7
 a| 
* Añadido link:../s3/quick-reference-support-for-aws-apis.html["Referencia rápida: Solicitudes de API de S3 admitidas"].
...
10,1
 a| 
Se añadió compatibilidad con la carga de archivos en varias partes, las solicitudes de tipo virtual-hosted-style y la autenticación v4.
...
|===
Consulta link:../s3/index.html#supported-versions["Versiones compatibles con la API de REST de S3 y actualizaciones"] para más información.
