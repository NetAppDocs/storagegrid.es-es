...
summary: La replicación entre redes consiste en la replicación automática de objetos entre buckets de S3 seleccionados en dos sistemas StorageGRID conectados mediante una conexión de federación de redes. La sincronización de cuentas es necesaria para la replicación entre redes. 
---
...
[role="lead"]
La replicación entre grids es la replicación automática de objetos entre buckets S3 seleccionados en dos sistemas StorageGRID que están conectados en una link:grid-federation-overview.html["conexión de federación de grid"]. link:grid-federation-what-is-account-sync.html["Sincronización de cuentas"] es necesario para la replicación entre grids.
...
image:grid_federation_cgr_workflow.png["Flujo de trabajo de replicación entre grid"]
...
Si una cuenta de inquilino tiene el permiso *Use grid federation connection* para usar una o más link:grid-federation-overview.html["conexiones de federación de grid"], un usuario del inquilino con permiso de acceso Root puede crear depósitos (depósitos "source" y "destination") en las cuentas de inquilino correspondientes en cada grid. Estos depósitos:

* Pueden tener nombres diferentes entre sí
* Puede tener diferentes regiones
* Debe tener el control de versiones activado
* Debe tener una de las siguientes configuraciones de S3 Object Lock:
+
** origen: *desactivado*, destino: *desactivado*
** origen: *activado*, destino: *activado*
** origen: *desactivado*, destino: *activado*




Después de que hayas creado ambos buckets, puedes configurar la replicación entre grids para cualquiera de los buckets o para ambos.
...
image:grid_federation_cross_grid_replication_one_direction.png["imagen que muestra una conexión de federación de cuadrícula en una dirección"]
...
image:grid_federation_cross_grid_replication.png["imagen que muestra la replicación en una dirección en comparación con la replicación en ambas direcciones"]
...
image:grid_federation_cross_grid_replication_delete.png["imagen que muestra la supresión del cliente de réplica en ambas cuadrículas"]
...
CAUTION: Puedes activar S3 Object Lock en buckets ya existentes, lo que podría dar lugar a una configuración incorrecta más adelante.


NOTE: Cuando S3 Object Lock está habilitado en el bucket de origen, asegúrate de que también lo esté en el bucket de destino. Si S3 Object Lock está deshabilitado en el bucket de destino, ocurre un error durante la replicación. Consulta link:../admin/grid-federation-troubleshoot.html#cross-grid-replication-alerts-and-errors["Alertas y errores de replicación entre grid"].
