...
[role="lead"]
Sigue el asistente de configuración de SSO para SAML y accede al modo de prueba para configurar y probar el inicio de sesión único (SSO) antes de habilitarlo para todos los usuarios de StorageGRID. Una vez habilitado el SSO, puedes volver al modo de prueba cuando sea necesario para modificar o volver a probar la configuración.
...
Para que StorageGRID (el proveedor de servicios) y el proveedor de identidad de SSO puedan comunicarse de forma segura sobre las solicitudes de autenticación de los usuarios, debes realizar estas tareas:
...
== [[enter-sandbox-mode]]Paso 2: proporciona confianzas de relying party, aplicaciones empresariales o conexiones de SP

Rellena los campos del paso 2 del asistente según el tipo de SSO que hayas seleccionado.

[role="tabbed-block"]
====
.Active Directory
--
Rellena los campos en el paso *Indicar el identificador de la parte que confía*.
...
.Identificación de entrada
--
Rellena los campos del paso *Configurar aplicaciones empresariales*.
...
. Sigue los pasos en link:../admin/creating-enterprise-application-entra-id-saml.html["Crear aplicaciones empresariales en Entra ID"] para crear una aplicación empresarial para cada Admin Node que aparece en la tabla.
...
.PingFederate
--
Rellena los campos del paso *Proporcionar ID de conexión SP*.
...
== Paso 3: Configura las relaciones de confianza con terceros, las aplicaciones empresariales o las conexiones con SP
...
+
Para obtener instrucciones, visita link:../admin/creating-relying-party-trusts-in-ad-fs-saml.html["Crear confianzas de parte de confianza en AD FS"].
...
. Sigue los pasos en link:../admin/creating-enterprise-application-entra-id-saml.html["Crear aplicaciones empresariales en Entra ID"] para cargar el archivo de metadatos SAML de cada nodo de administración en su aplicación empresarial de Entra ID correspondiente.
...
. Vaya a PingFederate.
. link:../admin/creating-sp-connection-ping-saml.html["Cree una o varias conexiones de proveedor de servicios (SP) para StorageGRID"]. Utiliza el ID de conexión SP de cada nodo de administración (que aparece en la tabla de la página Configurar SSO) y los metadatos SAML que descargaste para ese nodo de administración.
+
...
== [[test-sso]]Paso 4: probar la configuración
...
== [[enable-sso-saml]]Paso 5: Habilita el inicio de sesión único
...
.¿Qué sigue?
Después de configurar el inicio de sesión único usando SAML, configura el proveedor de identidad para los Admin Nodes.

[cols="1a,2a"]
|===
| Si tu proveedor de identidad es... | Tienes que... 


 a| 
Active Directory
 a| 
link:creating-relying-party-trusts-in-ad-fs-saml.html["Crea un grupo de aplicaciones para cada Admin Node"]
...
Identificación de entrada
 a| 
link:creating-enterprise-application-entra-id-saml.html["Crea una aplicación para cada Admin Node"]
...
PingFederate
 a| 
link:creating-sp-connection-ping-saml.html["Crea una conexión de proveedor de servicios para cada Admin Node"]
