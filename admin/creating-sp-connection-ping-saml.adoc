...
---
= Crea conexiones de proveedores de servicios en PingFederate para SAML en StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Utiliza PingFederate para crear una conexión de proveedor de servicios (SP) para cada nodo de administración para SAML en tu sistema. Para agilizar el proceso, importarás los metadatos SAML desde StorageGRID.

.Antes de empezar
* Ha configurado el inicio de sesión único para StorageGRID y ha seleccionado *Ping federate* como tipo de SSO.
* Tienes link:../admin/configure-sso-saml.html#enter-sandbox-mode["entró en modo sandbox"] en Grid Manager.
