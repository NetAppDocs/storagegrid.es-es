...
.Detalles
Para cada conexión de federación de grid, la columna *last error* muestra el error más reciente que se producirá, si lo hubiera, cuando los datos de un inquilino se estaban replicando en la otra cuadrícula. Esta columna muestra solo el último error de replicación entre cuadrículas que se produce; no se muestran los errores anteriores que podrían haberse producido. Un error en esta columna puede ocurrir por uno de estos motivos:
...
* El objeto de origen tiene S3 configuraciones de bloqueo de objetos que no cumplen con la configuración de retención de nivel de inquilino de la cuadrícula de destino.
* El objeto de origen tiene S3 configuraciones de bloqueo de objetos y S3 bloqueo de objetos está desactivado en el depósito de destino.
