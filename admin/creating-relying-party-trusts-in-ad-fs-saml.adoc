...
---
= Crea relaciones de confianza de partes confiables en AD FS para SAML en StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Debes utilizar los Servicios de federación de Active Directory (AD FS) para crear una relación de confianza de parte confiable para cada Admin Node de SAML en tu sistema. Puedes crear relaciones de confianza de parte confiable usando comandos de PowerShell, importando metadatos SAML desde StorageGRID o introduciendo los datos manualmente.

.Antes de empezar
* Ha configurado el inicio de sesión único para StorageGRID y ha seleccionado *AD FS* como tipo SSO.
* Tienes link:../admin/configure-sso-saml.html["entró en modo sandbox"] en Grid Manager.
...
. Cuando hayas terminado, vuelve a StorageGRID y link:../admin/configure-sso-saml.html#test-sso["Probar todos los fideicomisos de partes confiantes"] para comprobar que están configurados correctamente.
