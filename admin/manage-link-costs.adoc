...
---
= Administra los costes de enlace en StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* El servicio Load Balancer utiliza los costes de enlace en los nodos de administración y los nodos de puerta de enlace para dirigir las conexiones de los clientes. Consulta link:../admin/managing-load-balancing.html["Consideraciones que tener en cuenta al equilibrio de carga"].


El diagrama muestra una red de tres sitios en la que se han configurado los costes de enlace entre los sitios:

image:link_costs.png["Diagrama conceptual para vincular costes entre centros de datos"]
...
. Para cada sitio que figure en la columna Site/Link cost, introduce un valor de coste de 0 a 100 para los demás sitios.
+
No puede cambiar el coste del enlace si el origen es el mismo que el destino.

. Descartar o guardar los cambios:
+
** Para descartar los cambios, selecciona *Clear changes*.
