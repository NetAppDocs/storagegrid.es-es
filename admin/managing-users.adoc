...
Puede importar uno o más usuarios federados, hasta un máximo de 100 usuarios, directamente en la página Usuarios.

.Pasos
. Selecciona *CONFIGURACIÓN* > *USUARIOS ADMINISTRADOS*.
. Seleccione *Importar usuarios federados*.
. Introduzca el UUID o el nombre de usuario de uno o más usuarios federados.
+
Para varias entradas, agregue cada UUID o nombre de usuario en una nueva línea.

. Seleccione *Importar*.
+
Si la importación en el campo Usuarios falla para uno o más usuarios, realice los siguientes pasos:

+
.. Seleccione *Copiar usuarios*.
