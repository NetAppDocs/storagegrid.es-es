...
---
= Administra el almacenamiento de metadatos de objetos en StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
image:metadata_space_across_sites.png["Espacio de metadatos entre sitios"]
...
La configuración predeterminada para una nueva instalación de StorageGRID 12.1 o superior en la que cada Storage Node tenga 128 GB o más de RAM es de 20,000 GB (20 TB).
...
+
image:storage_used_object_metadata_actual_reserved.png["Almacenamiento usado - metadatos de objeto - reservado real"]



En la captura de pantalla, el valor Actual reserved es de 20 TB. Esta captura de pantalla corresponde a un Storage Node grande en una nueva instalación de StorageGRID 12.1. El volumen 0 tiene al menos 20 TB, así que usa la configuración predeterminada de espacio reservado para metadatos a nivel de sistema.



[[example-for-actual-reserved-metadata-space]]
=== Ejemplo de espacio real reservado para metadatos

Supongamos que instalas un nuevo sistema StorageGRID con la versión 12.1 o posterior. Para este ejemplo, supón que cada Storage Node tiene más de 128 GB de RAM y que el volumen 0 de Storage Node 1 (SN1) es de 6 TB. Según estos valores:

* El *espacio reservado para metadatos* de todo el sistema está configurado en 20 TB. (Este es el valor predeterminado para una nueva instalación de StorageGRID 12.1 o superior si cada Storage Node tiene más de 128 GB RAM.)
* El espacio *real reservado* para metadatos de SN1 es de 6 TB. (Se reserva todo el volumen porque el volumen 0 es más pequeño que la configuración de *Espacio reservado para metadatos*.)
...
image:metadata_allowed_space_volume_0.png["Metadatos volumen de espacio permitido 0"]
...
<= 4 TB
 a| 
75 % del espacio reservado real para metadatos, hasta un máximo de 1,32 TB
 a| 
El 75 % del espacio reservado para metadatos, hasta un máximo de 1,98 TB
...
> 4 TB
 a| 
(Espacio reservado para metadatos − 1 TB) × 75%, hasta un máximo de 1,32 TB
 a| 
(Espacio reservado para metadatos − 1 TB) × 75%, hasta un máximo de 7,92 TB
...
+
image:storage_used_object_metadata_allowed.png["Almacenamiento utilizado - metadatos de objeto - permitido"]



En la captura de pantalla, el valor *Permitido* es 7,92 TB, que es el valor máximo para un Storage Node cuyo espacio reservado para metadatos es superior a 4 TB.
...
* El *espacio reservado para metadatos* de todo el sistema está configurado en 20 TB. (Este es el valor predeterminado para una nueva instalación de StorageGRID 12.1 o superior si cada Storage Node tiene más de 128 GB RAM.)
* El espacio *real reservado* para metadatos de SN1 es de 6 TB. (Se reserva todo el volumen porque el volumen 0 es más pequeño que la configuración de *Espacio reservado para metadatos*.)
* El espacio *permitido* para metadatos en SN1 es de 3,75 TB, según el cálculo que figura en la <<table-allowed-space-for-metadata,tabla para el espacio permitido para los metadatos>>: (espacio reservado para metadatos − 1 TB) × 75 %, hasta un máximo de 7,92 TB.
...
image:metadata_space_three_storage_nodes.png["Espacio de metadatos tres nodos de almacenamiento"]
