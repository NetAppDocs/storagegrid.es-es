...
Para obtener más información sobre el uso de los controles del cortafuegos, incluidos ejemplos, consulta link:../admin/manage-firewall-controls.html["Gestionar los controles del firewall"].

Para obtener más información sobre los cortafuegos externos y la seguridad de la red, consulta link:../admin/controlling-access-through-firewalls.html["Controle el acceso a un firewall externo"].
...
NOTE: La configuración de la pestaña «Lista de direcciones privilegiadas» puede anular la configuración de la pestaña «Red de clientes no fiables».
...
. Si lo deseas, selecciona *Add another IP o una subred en notación CIDR* para añadir clientes con privilegios adicionales.
+

TIP: Agregue el menor número posible de direcciones a la lista de privilegios.

. Si lo deseas, selecciona *Permitir que las direcciones IP con privilegios accedan a los puertos internos de StorageGRID*. Consulta link:../network/internal-grid-node-communications.html["Puertos internos StorageGRID"].
+
...
CAUTION: Si cierras los puertos 443 o 8443 de Grid Manager, cualquier usuario que esté conectado en ese momento a un puerto bloqueado, incluido tú, perderá el acceso a Grid Manager a menos que su dirección IP se haya añadido a la lista de direcciones privilegiadas.
