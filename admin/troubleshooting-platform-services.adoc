...
summary: Los puntos finales utilizados en los servicios de la plataforma son creados y gestionados por los usuarios del inquilino en el Tenant Manager. Sin embargo, si un inquilino tiene problemas para configurar o usar los servicios de la plataforma, podrías usar el Grid Manager para ayudar a resolver el problema. 
---
...
image:tenant_dashboard_endpoint_error.png["mensaje de error de punto final en el panel de control"]

Los usuarios de Tenant pueden acceder a la página Endpoints para consultar el mensaje de error más reciente de cada endpoint y determinar cuánto tiempo hace que ocurrió el error. La columna *Last error* muestra el mensaje de error más reciente de cada endpoint e indica cuánto tiempo hace que ocurrió el error. Los errores que incluyen el icono image:icon_alert_red_critical.png["Icono X rojo"] ocurrieron en los últimos 7 días.

image:endpoints_last_error.png["Captura de pantalla de la página endpoints que muestra la columna Last error"]
...
+
image:nodes_page_site_level_platform_services.gif["Página nodos Servicios de plataforma a nivel de sitio"]
