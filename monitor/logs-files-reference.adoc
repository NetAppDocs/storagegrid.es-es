...
---
= Archivos de registro de StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
| acct-tunnel.log  a| 
Registro del asistente de túnel de la cuenta que funciona con el balanceador de carga.
...
Registro de tráfico del proxy HTTP interno.
...
| manage-audit.log  a| 
`mgmt-api` acceder a los registros de auditoría.
...
* stream-access.log: Sesiones de proxy de stream TCP/TLS (capa 4) para el bloque de stream.
+
*Nota*: Esto no es un registro de solicitudes HTTP.
...
Registro de acceso para el tráfico de la federación de grid que se enruta como S3 (espacio de nombres global/URLs de federación).
...
| system-bucket.log  a| 
`update-system-bucket-configuration.log`: Registros del bucket del sistema.
...
`update-cors-configurations.log`: Registra los logs relacionados con los cambios en TLS/cifrado y CORS.



| update-leakd-configurations.log  a| 
Registro de configuración de la detección de archivos huérfanos.
