...
---
= Ver la pestaña Overview de StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
image:nodes_page_overview_tab.png["Pestaña Nodes Page Overview"]
...
+
** *Desconocido* image:icon_alarm_blue_unknown.png["icono de signo de interrogación azul"]: Por una razón desconocida, el nodo no está conectado a la red o uno o más servicios están inactivos de forma inesperada. Por ejemplo, se ha perdido la conexión de red entre los nodos, hay un corte de corriente o un servicio está inactivo. También podría activarse la alerta *Unable to communicate with node*. Es posible que haya otras alertas activas. Esta situación requiere atención inmediata.
+
...
** *Fuera de servicio por motivos administrativos* image:icon_alarm_gray_administratively_down.png["icono de questionmark gris"]: el nodo no está conectado a la grid por un motivo previsto. Por ejemplo, el nodo, o los servicios en el nodo, se ha apagado de forma controlada, el nodo se está reiniciando o el software se está actualizando. También puede haber una o más alertas activas.
** *Conectado* image:icon_alert_green_checkmark.png["icono de alerta verde marca de verificación"]: el nodo está conectado a la red.
...
* *Direcciones IP*: Las direcciones IP del nodo. Selecciona *Mostrar direcciones IP adicionales* para ver las direcciones IPv4 e IPv6 del nodo y las asignaciones de interfaces.
...
image:nodes_page_alerts_table.png["Tabla de alertas de página nodos"]
