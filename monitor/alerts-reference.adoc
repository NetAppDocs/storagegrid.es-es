...
Fallo de la batería CMOS del aparato
 a| 
Se detectó un problema con la batería CMOS del appliance.
...
La versión del software SANtricity es inferior al mínimo recomendado para esta versión de StorageGRID.
 ...
 a| 
Se ha detectado un fallo en el SFP del appliance
 a| 
Se ha detectado un fallo de SFP en el controlador de cómputo de un dispositivo StorageGRID.
...
Un bucket de FabricPool tiene activado el control de versiones o el bloqueo de objetos S3, que no están soportados.
 ...
 a| 
ListBucket comprobación de permisos para el objeto Get/Head desactivada
 a| 
ListBucket la comprobación de permisos para GetObject/HeadObject en claves inexistentes está desactivada.
...
| Nombre de alerta | Descripción 
 ...
 a| 
Servicio Chunk con un elevado número de goroutines
 a| 
El servicio «chunk» tiene un número inusualmente alto de goroutines, lo que podría indicar un interbloqueo o una fuga de recursos.
...
Ya no es posible verificar los fragmentos de código de borrado. Es posible que los fragmentos corruptos no se reparen.
 ...
 a| 
Grupos con código de borrado en estado Needs Repair
 a| 
Uno o más grupos con código de borrado están en estado de "Necesita reparación" cuando no hay ninguna reparación de datos con código de borrado en curso.
...
Se han detectado objetos huérfanos.
 ...
 a| 
El tamaño de una parte de la subida multiparte a S3 es demasiado grande
 a| 
Un cliente está intentando realizar una operación de carga en varias partes que supera el límite de tamaño por parte.
...
Un cliente está intentando realizar una operación PUT Object que supera los S3 límites de tamaño.
 ...
 a| 
Solicitudes S3 que utilizan encabezados condicionales no compatibles
 a| 
Un cliente S3 está utilizando encabezados condicionales no compatibles (If-Match o If-None-Match) para determinadas operaciones.



 a| 
Solicitudes a S3 que utilizan una región incorrecta con el espacio de nombres global activado
 a| 
Un cliente de S3 está utilizando una región de firma incorrecta para acceder a los buckets de este grid.
