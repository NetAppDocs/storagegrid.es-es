...
[role="lead"]
Según sea necesario, configura los niveles de auditoría, los encabezados de protocolo, la ubicación de los mensajes y registros de auditoría, y la retención de los registros.
...
Si lo deseas, puedes link:../monitor/considerations-for-external-syslog-server.html["configurar un servidor syslog externo"] para guardar la información de auditoría de forma remota. Puedes minimizar el impacto en el rendimiento del registro de mensajes de auditoría sin reducir la exhaustividad de los datos de auditoría usando un servidor externo. Un servidor syslog externo es especialmente útil si tienes una grid grande, usas varios tipos de aplicaciones S3 o quieres conservar todos los datos de auditoría.
...
+
Cuando StorageGRID detecta encabezados HTTP en una solicitud, los incluye en el mensaje de auditoría en el campo HTRH.
...
De forma predeterminada, los mensajes de auditoría y los registros se guardan en los nodos donde se generan. StorageGRID rota periódicamente y, finalmente, elimina los mensajes de auditoría y los registros para evitar que ocupen un espacio excesivo en disco. Para guardar los mensajes de auditoría y un subconjunto de registros de forma externa, <<use-external-syslog-server,utilizar un servidor syslog externo>>.
...
+
Si seleccionas un valor, StorageGRID aplica dicho valor a todos los mensajes de este tipo. Pierdes detalles sobre las diferentes gravedades si sobrescribes la gravedad con un valor fijo.
...
Antes de utilizar un servidor syslog externo, solicita que todos los nodos de tu grid envíen mensajes de prueba al servidor syslog externo. Estos mensajes de prueba te ayudan a validar toda tu infraestructura de recopilación de registros antes de comprometerte a enviar datos al servidor syslog externo.
...
== Gestiona la retención de registros

Puedes gestionar el tiempo que StorageGRID conserva los registros creando una regla de ILM o configurando una política de ciclo de vida del bucket de S3.

[role="tabbed-block"]
====
.Regla de ILM
--
Crea una regla de ILM para el bucket y añade esta regla a la política de ILM activa.

.Pasos
. Selecciona *ILM* > *Reglas*.
. Selecciona *Crear*. link:../ilm/create-ilm-rule-enter-details.html["Paso 1 (Introducir detalles)"] de la pantalla para crear una regla de ILM del asistente aparece:
+
.. Introduce un nombre único para la regla. Por ejemplo: Regla ILM para registros archivados.
.. Selecciona la cuenta de S3 del inquilino para el archivado de registros.
.. Especifica el depósito de S3 para el archivado de registros.


. Accede a link:../ilm/create-ilm-rule-define-placements.html["Paso 2 (Definir ubicaciones)"] y define la ubicación de la regla de registros archivados. Por ejemplo, la regla ILM almacena una copia replicada en Site 1 y otra en Site 2 durante el primer año. Después de un año, se crea una copia con código de borrado 2+1 y se guarda únicamente en un sitio.
. Después de crear la regla, añádela a una política de ILM activa.


--
.Ciclo de vida de un bucket de S3
--
Configura una política de ciclo de vida de un bucket S3 en el bucket S3 de archivo de registros:
...
----
Al configurar una política de ciclo de vida de un bucket de S3 en el bucket de S3 destinado al archivo de registros, las acciones de caducidad del ciclo de vida anulan la política de ILM para los objetos que coinciden con el filtro de ciclo de vida del bucket. Los objetos que no coinciden con el filtro de ciclo de vida del bucket usan los ajustes de retención de la política de ILM. Si un objeto coincide con el filtro de ciclo de vida del bucket y no se especifican explícitamente acciones de caducidad, no se usan los ajustes de retención de la política de ILM y se da por supuesto que las versiones del objeto se conservan para siempre.
