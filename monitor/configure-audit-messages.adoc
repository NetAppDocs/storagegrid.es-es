...
Error
 a| 
Solo se registran los mensajes de error: Mensajes de auditoría para los que el código de resultado no es correcto (SUCS).
...
NOTE: Los encabezados de solicitud de protocolo de auditoría se registran solo si el nivel de auditoría para *Client reads* o *Client Writes* no está *Off*.
...
NOTE: Si está utilizando UDP, compruebe toda su infraestructura de recopilación de registros. El protocolo UDP no permite una detección de errores tan rigurosa como los otros protocolos.
