...
---
= Operaciones de carga multiparte compatibles en StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Si un objeto cargado en varias partes se incluye en un link:../tenant/what-is-branch-bucket.html["cubo de rama"] depende de la hora en que se inició la ingesta y del filtro de tiempo para el bucket de la rama:
+
** *Antes*. Si la importación se inició antes de la marca de tiempo _Before_ del bucket de la rama, se incluye el objeto subido en varias partes, independientemente de cuándo finalice la subida.
** *Después*. Si la importación se inició después de la marca de tiempo _After_ del bucket de la rama, se incluye el objeto subido en varias partes.
** *Entre*. Si la ingesta se inició entre las marcas de tiempo _From_ y _To_ del bucket de la rama, se incluye el objeto subido en varias partes, independientemente de cuándo finalice la subida.
** *Excluir*. Si la importación se inició antes de la hora indicada en el campo _Exclude from_ del bucket de la rama o después de la hora indicada en el campo _Exclude to_, se incluye el objeto subido en varias partes.
