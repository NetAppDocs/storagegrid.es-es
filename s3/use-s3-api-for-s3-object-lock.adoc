...
NOTE: StorageGRID no admite el encabezado de solicitud  `x-amz-sdk-checksum-algorithm`.
...
Para habilitar S3 Object Lock en un bucket, utiliza uno de estos métodos:
...
* Gestiona la configuración del bucket desde el Tenant Manager. Consulta link:../tenant/update-default-retention-settings.html["Actualizar S3 Retención predeterminada de bloqueo de objetos"].
...
Cuando se importan nuevas versiones de objetos al depósito, se aplica el modo de retención predeterminado si `x-amz-object-lock-mode` y `x-amz-object-lock-retain-until-date` no se especifican. El periodo de retención predeterminado se utiliza para calcular la retain-until-date si `x-amz-object-lock-retain-until-date` no se especifica.

Si se modifica el periodo de retención predeterminado tras la incorporación de una versión de un objeto, la fecha de retención de dicha versión del objeto sigue siendo la misma y no se vuelve a calcular usando el nuevo periodo de retención predeterminado.

Debe tener `s3:PutBucketObjectLockConfiguration` el permiso, o ser account root, para completar esta operación.

Debes especificar uno de los siguientes encabezados de solicitud en la solicitud PutObjectLockConfiguration:
...
NOTE: StorageGRID no valida el contenido del encabezado x-amz-checksum-* para esta operación.



==== Ejemplo de solicitud

Este ejemplo habilita S3 Object Lock para un bucket (si tu administrador de grid te ha dado permiso) y establece el modo de retención predeterminado en COMPLIANCE y el periodo de retención predeterminado en 6 años.
...
* Debes incluir uno de los siguientes encabezados de solicitud si hay algún encabezado de solicitud x-amz-object-lock-* presente en la solicitud PutObject:
+
[listing]
----
Content-MD5
...
----
+
Estos encabezados no son necesarios para CopyObject ni para CreateMultipartUpload.

* Si el bucket no tiene habilitado S3 Object Lock y se incluye un encabezado de solicitud x-amz-object-lock-*, se devuelve un error 400 Bad Request (InvalidRequest).
...
Debes especificar uno de los siguientes encabezados de solicitud en la solicitud:
...
NOTE: StorageGRID no valida el contenido del encabezado x-amz-checksum-* en estas operaciones.
...
+
El encabezado  `x-amz-bypass-governance-retention` no tiene ningún efecto en los objetos sin S3 Object Lock.
