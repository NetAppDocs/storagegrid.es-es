...
summary: El sistema StorageGRID admite un máximo de 20,000 buckets por cada cuenta de inquilino S3. 
---
...
[role="lead"]
El sistema StorageGRID admite un máximo de 20,000 buckets por cada cuenta de inquilino S3.

Cada rejilla puede tener un máximo de 100.000 cubos.

Si varios tenants tienen hasta 5,000 buckets cada uno, cada Storage Node en el grid debe tener un mínimo de 64 GB de RAM.

Las restricciones relativas a los nombres de los buckets se ajustan a las de la región estándar de AWS US, pero deberías restringirlos aún más para que sigan las convenciones de nomenclatura de DNS y así admitir solicitudes de S3 con estilo virtual-hosted.
...
Puedes comprobar si las actualizaciones del tiempo de acceso están activadas o desactivadas para cada depósito. Consulta link:get-bucket-last-access-time-request.html["GET Bucket última hora de acceso"].
...
*Nota*: No se admiten los buckets de directorios de S3.
...
* Puedes incluir el encabezado de solicitud  `x-amz-bucket-object-lock-enabled` para crear un bucket con S3 Object Lock habilitado. Consulta link:../s3/use-s3-api-for-s3-object-lock.html["Use la API REST DE S3 para configurar el bloqueo de objetos de S3"].
+
S3 Object Lock requiere el control de versiones del bucket, que se activa automáticamente al crear el bucket con S3 Object Lock habilitado.

+
Puedes activar S3 Object Lock al crear el bucket o más adelante.
...
DeleteBucketLifecycle
 a| 
Elimina la configuración del ciclo de vida del depósito. Consulta link:create-s3-lifecycle-configuration.html["Cree una configuración del ciclo de vida de S3"].
...
(Anteriormente llamado GET Bucket Lifecycle)
 a| 
Devuelve la configuración del ciclo de vida del depósito. Consulta link:create-s3-lifecycle-configuration.html["Cree una configuración del ciclo de vida de S3"].
...
GetBucketLocation
 a| 
Devuelve la región que se ha establecido mediante el elemento  `LocationConstraint` en la solicitud CreateBucket. Si la región del bucket es  `us-east-1` (por defecto), se devuelve un valor vacío para  `LocationConstraint`.
...
*Precaución*: Si se ha configurado una etiqueta de política de ILM no predeterminada para este bucket, habrá una etiqueta de bucket  `NTAP-SG-ILM-BUCKET-TAG` con un valor asignado. No modifiques ni elimines esta etiqueta.
...
Consulta link:../s3/use-s3-api-for-s3-object-lock.html["Use la API REST DE S3 para configurar el bloqueo de objetos de S3"].
...
Consulta link:create-s3-lifecycle-configuration.html["Cree una configuración del ciclo de vida de S3"]. Para entender cómo la acción de caducidad en el ciclo de vida de un bucket interactúa con las instrucciones de ubicación de ILM, consulta link:../ilm/how-ilm-operates-throughout-objects-life.html["Cómo funciona ILM a lo largo de la vida de un objeto"].
...
Política de PutBucketPolicy
 a| 
Establece la política asociada al depósito. Consulta link:use-access-policies.html["Utilice las políticas de acceso de bloques y grupos"].
...
* Los buckets de destino deben especificarse como el URN de los endpoints de StorageGRID, tal y como se indica en Tenant Manager o en la Tenant Management API. Consulta link:../tenant/configuring-cloudmirror-replication.html["Configure la replicación de CloudMirror"].
+
...
* Si el administrador de la grid ha dado su autorización, activa S3 Object Lock en los buckets existentes.


Si se modifica el periodo de retención predeterminado, la fecha de retención de las versiones existentes de los objetos sigue siendo la misma y no se vuelve a calcular usando el nuevo periodo de retención predeterminado.

Consulta link:../s3/use-s3-api-for-s3-object-lock.html["Use la API REST DE S3 para configurar el bloqueo de objetos de S3"] para obtener información detallada.
