...
Las operaciones ListObjects (GET Bucket) y ListObjectVersions (GET Bucket object versions) admiten StorageGRID link:consistency.html["valores de coherencia"].
...
* StorageGRID admite temas de Amazon Simple Notification Service (Amazon SNS), temas de Kafka o puntos finales de webhook como destinos. No se admiten el servicio de cola simple (SQS) ni los extremos de AWS Lambda.
