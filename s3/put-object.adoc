...
* `Content-Encoding`
+
Cuando especificas  `aws-chunked` para  `Content-Encoding`, StorageGRID no verifica los siguientes elementos:

+
** StorageGRID no verifica la  `chunk-signature` con los datos del fragmento.
** StorageGRID no verifica el valor que proporcionas para `x-amz-decoded-content-length` con respecto al objeto.
...
NOTE: Si una solicitud PutObject no especifica una suma de comprobación, StorageGRID no calcula una suma de comprobación CRC-64/NVME por defecto para un objeto.
