...
* StorageGRID no devuelve el encabezado  `x-amz-missing-meta` si el valor interpretado del nombre o del valor de la clave incluye caracteres no imprimibles.
...
* `Content-Type`
* `x-amz-checksum-algorithm`: Indica `CRC32`, `CRC32C`, `CRC64NVME` o `SHA256`
+
Cuando copias un objeto, si el objeto de origen tiene un  `checksum` o incluye  `x-amz-checksum-algorithm`, el objeto copiado tiene una suma de comprobación FULL_OBJECT.

+
Si el objeto de origen tiene una suma de comprobación pero la solicitud no incluye  `x-amz-checksum-algorithm` o utiliza un valor de suma de comprobación no compatible, el objeto copiado tiene el mismo tipo de suma de comprobación que el objeto de origen. StorageGRID ignora los valores  `x-amz-checksum-algorithm` no válidos o no compatibles.

+
Si el objeto de origen no tiene suma de comprobación y no incluye  `x-amz-checksum-algorithm` o utiliza un valor de suma de comprobación no compatible, el objeto copiado no tiene suma de comprobación.

+

NOTE: Si una solicitud CopyObject no especifica una suma de comprobación y el objeto de origen no tiene una suma de comprobación, StorageGRID no calcula una suma de comprobación CRC-64/NVME por defecto para un objeto.
...
* `If-Match`
+
El  `If-Match` encabezado se comporta de forma diferente según tu configuración de StorageGRID. Si proporcionas este encabezado, podría devolverse un mensaje de error (501 Not Implemented). Si el  `If-Match` encabezado se acepta pero no funciona, podría activarse una alerta con instrucciones sobre cómo habilitar el comportamiento más estricto del error 501 Not Implemented.

* `If-None-Match`
+
El  `If-None-Match` encabezado se comporta de forma diferente según tu configuración de StorageGRID. Incluir este encabezado podría devolver un mensaje de error (501 Not Implemented). Si el  `If-None-Match` encabezado se acepta pero no funciona, podría activarse una alerta con instrucciones sobre cómo habilitar el comportamiento más estricto del error 501 Not Implemented.
...
Si el origen y el destino coinciden, y el encabezado  `x-amz-metadata-directive` se especifica como  `REPLACE`, los metadatos del objeto se actualizan con los valores de metadatos proporcionados en la solicitud. En este caso, StorageGRID no vuelve a incorporar el objeto. Esto tiene dos consecuencias importantes:
...
* No se utiliza la opción de «Comportamiento de ingesta» especificada en la regla de ILM correspondiente. Cualquier cambio en la ubicación de los objetos que se derive de la actualización se realiza cuando los procesos normales de ILM en segundo plano vuelven a evaluar el ILM.
+
