...
---
= Cómo StorageGRID implementa las operaciones de API de REST de S3 para objetos
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Los encabezados  `If-Match` y  `If-None-Match` se comportan de forma diferente según tu configuración de StorageGRID. Incluir estos encabezados podría devolver un mensaje de error (501 Not Implemented). Si los encabezados  `If-Match` o  `If-None-Match` se aceptan pero no funcionan, podría activarse una alerta con instrucciones sobre cómo habilitar el comportamiento más estricto del error 501 Not Implemented.
