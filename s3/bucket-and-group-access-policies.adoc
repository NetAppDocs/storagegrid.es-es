...
IfExists
 a| 
Agregado a cualquier operador de condición, excepto la condición Nulo, para comprobar la ausencia de esa clave de condición. Devuelve TRUE si la clave de condición no está presente.
...
s3:PutObject
 a| 
Se compara con el `object-lock-mode` expandido desde la cabecera de solicitud en la solicitud PutObject, CopyObject y CreateMultipartUpload.
...
s3:PutObject
 a| 
Se compara con `sse-customer-algorithm` o con `copy-source-sse-customer-algorithm` el expandido desde el encabezado de solicitud en las solicitudes PutObject, CopyObject, CreateMultipartUpload, UploadPart, UploadPartCopy y CompleteMultipartUpload.
