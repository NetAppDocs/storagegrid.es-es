...
summary: 'StorageGRID utiliza el lenguaje de políticas de Amazon Web Services (AWS) para permitir a los inquilinos de S3 controlar el acceso a los buckets y a los objetos dentro de esos buckets.' 
---
...
* Los caracteres internacionales, que pueden especificarse en la clave del objeto, deben codificarse usando JSON UTF-8 o usando las secuencias de escape JSON \u. No se admite la codificación percent-encoding.
+
...
* El valor principal puede especificar un nombre de grupo o usuario que aún no exista cuando se crea una política de bucket.
...
== [[use-putoverwriteobject-permission]]Usa el permiso PutOverwriteObject
...
* Cuando no existe este permiso, el efecto es el mismo que si Allow estuviera activado.
...
Operadores IP
 a| 
Compara con la dirección IP desde la que se envió la solicitud. Se puede utilizar para operaciones con buckets u objetos.

*Nota:* Si la solicitud S3 se envió a través del servicio de Load Balancer en los nodos de administración y los nodos de Gateway, esto se compara con la dirección IP situada aguas arriba del servicio de Load Balancer.

*Nota*: Si se utiliza un equilibrador de carga de terceros no transparente, se compara con la dirección IP de ese equilibrador de carga. Cualquier encabezado  `X-Forwarded-For` se ignora porque no se puede verificar su validez.
...
Recurso/identidad
 a| 
Se compara con el nombre de usuario del remitente desde el que se envió la solicitud. Se puede utilizar para operaciones con depósitos u objetos.
...
s3:ListBucketVersions permisos
 a| 
Compara con el parámetro delimitador especificado en una solicitud ListObjects o ListObjectVersions.
...
s3:PutObjectVersionEtiquetado
 a| 
Requiere que el objeto existente tenga la clave y el valor específicos de la etiqueta.
...
s3:ListBucketVersions permisos
 a| 
Compara con el parámetro de prefijo especificado en una solicitud ListObjects o ListObjectVersions, o con la clave especificada para GetObject o HeadObject.
...
s3:PutObjectVersionEtiquetado
 a| 
Requiere una clave y un valor de etiqueta específicos cuando la solicitud del objeto incluye el etiquetado.
...
s3:PutObject
 a| 
Se compara con el valor x-amz-copy-source extraído del encabezado de la solicitud en la solicitud PutObject y UploadPartCopy.
...
Cucharón
 a| 
Válido. Los objetos son propiedad de la cuenta del creador y la política del bucket no se aplica. La cuenta del creador debe conceder permisos de acceso al objeto mediante las ACL de objeto.
 a| 
...
NOTE: Establecer DeleteObject en DENY en una política de S3 no impide que ILM elimine objetos cuando existe una regla como "cero copias después de 30 días".
