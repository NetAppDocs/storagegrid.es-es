...
---
= Carga una parte de un objeto en StorageGRID con la operación S3 UploadPartCopy
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[[s3-checksum-support]]
== Compatibilidad con la suma de comprobación de S3

StorageGRID detecta automáticamente si se ha especificado una suma de comprobación durante una solicitud CreateMultipartUpload y utiliza esta suma de comprobación en la operación UploadPartCopy. No necesitas especificar una suma de comprobación en una solicitud UploadPartCopy. Si una solicitud CreateMultipartUpload no especifica una suma de comprobación, StorageGRID no calcula una suma de comprobación CRC-64/NVME predeterminada para un objeto. StorageGRID no incluye una suma de comprobación para la parte copiada cuando CreateMultipartUpload no ha especificado una suma de comprobación.
