...
---
= Crear un bucket S3 de StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Si hay varios inquilinos y cada uno de ellos tiene hasta 5,000 buckets, cada nodo de almacenamiento en el grid tiene un mínimo de 64 GB de RAM.
+

NOTE: El sistema StorageGRID admite un máximo de 20 000 buckets por cada cuenta de inquilino de S3. Cada grid puede tener un máximo de 100 000 buckets, incluyendo link:../tenant/what-is-branch-bucket.html["cubos de ramas"]. También hay un límite para link:../tenant/manage-branch-buckets.html["profundidad del bucket de la rama"].
...
Para obtener más información, consulta https://docs.aws.amazon.com/AmazonS3/latest/userguide/bucketnamingrules.html["Documentación de Amazon Web Services (AWS) sobre reglas de nomenclatura de bloques"^].
