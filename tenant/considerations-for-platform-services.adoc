...
Se puede configurar cualquier combinación de servicios de la plataforma para un único bucket de S3. Por ejemplo, podrías configurar tanto el link:../tenant/understanding-cloudmirror-replication-service.html["Servicio CloudMirror"] como el link:../tenant/understanding-notifications-for-buckets.html["notificaciones"] en un bucket de S3 de StorageGRID, para que puedas reflejar objetos específicos en Amazon Simple Storage Service (S3) y, al mismo tiempo, enviar una notificación sobre cada uno de esos objetos a una aplicación de monitorización de terceros que te ayude a hacer un seguimiento de tus gastos en AWS.
...
Al utilizar el control de versiones, ten en cuenta que el orden de las versiones de los objetos en el bucket de destino es un esfuerzo máximo y no está garantizado por el servicio CloudMirror, debido a las limitaciones del protocolo S3. Además, los buckets de destino mantendrán su propia configuración y puede que no proporcionen control de versiones.
...
Bloque con S3 Object Lock habilitado
 a| 
Si el bucket S3 de origen en StorageGRID tiene habilitado S3 Object Lock, también debes habilitar S3 Object Lock para el bucket de destino con la retención predeterminada configurada.

La replicación de _datos de objetos_ es compatible con los buckets de origen y destino que tengan habilitado S3 Object Lock. Sin embargo, los buckets de destino mantendrán su propia configuración para la retención de objetos y podrían no ofrecer cumplimiento ni control de versiones.
