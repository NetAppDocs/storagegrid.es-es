...
---
= Configura la replicación CloudMirror en StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* La replicación solo es compatible con buckets de origen o de destino que tengan habilitado S3 Object Lock para buckets de Amazon S3 o StorageGRID S3. Los encabezados de object lock no se replican en el objeto de destino.
...
* Si activas S3 Object Lock en el bucket de destino después de configurar la replicación de CloudMirror, es posible que tengas que link:../tenant/editing-platform-services-endpoint.html["volver a guardar el endpoint"] para que la replicación pueda reanudarse.
