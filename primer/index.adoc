...
image:storagegrid_system_diagram.png["Imagen con múltiples clientes"]
...
* Compatibilidad con implementaciones basadas en software o en hardware. Puedes implementar StorageGRID en cualquiera de las siguientes opciones:
+
...
* Gestión federada de identidades. Se integra con Active Directory, OpenLDAP u Oracle Directory Service para la autenticación de usuarios. Admite el inicio de sesión único (SSO) usando Security Assertion Markup Language 2.0 (SAML 2.0) u OpenID Connect (OIDC) para intercambiar datos de autenticación y autorización entre StorageGRID y un proveedor de identidades externo como Active Directory Federation Services (AD FS).
