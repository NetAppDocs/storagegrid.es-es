...
De forma predeterminada, todos los nuevos nodos de almacenamiento almacenan tanto los datos de los objetos como los metadatos. Este tipo de nodo de almacenamiento se llama nodo de almacenamiento _combinado_.
...
* Al utilizar dispositivos StorageGRID, solo puedes configurar nodos exclusivos de metadatos en dispositivos SGF6112 con doce unidades de 1,9 TB o doce de 3,8 TB.
* Al utilizar nodos basados en software, debes asegurarte de que los recursos de los nodos solo de metadatos coincidan con los recursos de los Storage Nodes existentes. Por ejemplo:
+
...
Puedes convertir un nodo de almacenamiento combinado en un nodo de almacenamiento solo de datos. Si el nodo de almacenamiento contiene el <<adc-service,Servicio ADC>>, y se cumplen los requisitos previos para la conversión, el servicio se traslada automáticamente a otro nodo de almacenamiento en el mismo sitio durante la conversión. Para conocer los requisitos previos y los pasos detallados, consulta link:../maintain/convert-to-data-only-node.html["Convertir un nodo de almacenamiento en un nodo de solo datos"].



[[required-storage-nodes-per-grid-and-per-site]]
...
El servicio ADC gestiona la información sobre la topología, incluida la ubicación y la disponibilidad de los servicios. Cuando un nodo de grid necesita información de otro nodo de grid o que otro nodo de grid realice una acción, se pone en contacto con un servicio ADC para encontrar el nodo de grid más adecuado para procesar su solicitud. Además, el servicio ADC conserva una copia de los paquetes de configuración de la implementación de StorageGRID, lo que permite a cualquier nodo de grid recuperar la información de configuración actual.
