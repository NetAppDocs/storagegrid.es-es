...
image:replication_data_flow.png["diagrama que muestra el proceso descrito en el texto circundante"]
...
image:erasure_coding_data_flow.png["Flujo de datos de código de borrado"]
...
Si las instrucciones de ubicación de contenido de una regla de ILM exigen que se almacene una copia replicada de los datos de objetos en un Cloud Storage Pool, los datos de objetos se duplican en el bucket externo de S3 o en el contenedor de Azure Blob Storage que se haya especificado para el Cloud Storage Pool.
...
image:cloud_storage_pool_data_flow.png["Flujo de datos del pool de almacenamiento del cloud"]
