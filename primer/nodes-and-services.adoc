...
+
--
Durante la instalación inicial de un nuevo nodo de almacenamiento, puedes especificar que solo se utilice para link:../primer/what-storage-node-is.html#types-of-storage-nodes["almacenar metadatos o datos"].
...
* https://docs.netapp.com/us-en/storagegrid-appliances/["Documentación de la appliance StorageGRID"^]
...
gdu-server
 a| 
El ejecutor de tareas de backend para las acciones de mantenimiento y aprovisionamiento de grid. Acepta solicitudes de `mgmt-api` y otros scripts de utilidades, las valida y envía cada acción al gestor de tareas GDU correspondiente, como aprovisionamiento, expansión, decomisionar y actualizaciones.
 a| 
Nodos de administración
...
Alta disponibilidad
 a| 
Gestiona las direcciones IP virtuales de alta disponibilidad en los nodos configurados en la página Grupos de alta disponibilidad. Este servicio también se conoce como el servicio _keepalived_.
 a| 
...
mgmt-api
 a| 
La API de backend de Grid Manager. Expone endpoints REST autenticados para administrar el grid (configuración, monitoreo y flujos de trabajo de mantenimiento), aplica validaciones y permisos, y orquesta operaciones de larga duración. La API principal del plano de control utilizada por la interfaz de Grid Manager y los clientes de automatización.
 a| 
Nodos de administración
