...
NOTE: Si el link:enabling-s3-object-lock.html["Se ha activado la configuración de bloqueo de objeto global S3"], debes asegurarte de que la política de ILM cumpla con los requisitos de los buckets que tienen habilitado S3 Object Lock. En esta sección, sigue las instrucciones que mencionan tener S3 Object Lock habilitado.
...
--
** El link:enabling-s3-object-lock.html["La configuración de bloqueo de objetos global S3 ya está activada"] para el sistema StorageGRID.
...
.Resultados
Cualquier modificación en las reglas de la política se reflejará en los resultados de la simulación y mostrará la nueva coincidencia y la coincidencia anterior. La ventana Simular policy conserva los objetos que probaste hasta que selecciones *Clear all* o el icono de eliminar image:icon_x_to_remove.png["Icono Eliminar"] para cada objeto en la lista de resultados de la simulación.
