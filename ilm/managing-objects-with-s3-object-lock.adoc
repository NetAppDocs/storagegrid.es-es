...
Cuando la configuración global de S3 Object Lock está habilitada para un sistema StorageGRID, una cuenta de inquilino de S3 puede crear buckets con o sin S3 Object Lock habilitado. Cuando la configuración permitir S3 Object Lock para buckets existentes está habilitada, una cuenta de inquilino de S3 puede habilitar S3 Object Lock para un bucket existente. Si un bucket tiene S3 Object Lock habilitado, el versionado del bucket es obligatorio y se habilita automáticamente. Después de que una cuenta de inquilino de S3 habilita S3 Object Lock para un bucket nuevo o existente, ya no se puede deshabilitar.
...
+
Para obtener más información sobre la configuración del objeto, consulta link:../s3/use-s3-api-for-s3-object-lock.html["Use la API REST DE S3 para configurar el bloqueo de objetos de S3"].
...
Consulta link:../tenant/creating-s3-bucket.html["Cree un bloque de S3"] y link:../tenant/update-default-retention-settings.html["Actualizar S3 Retención predeterminada de bloqueo de objetos"].
