...
Consulta link:using-multiple-storage-pools-for-cross-site-replication.html["Ejemplos de habilitación de la protección contra pérdida de sitios mediante replicación y código de borrado"].
...
+
La disminución del rendimiento afecta las velocidades máximas alcanzables de incorporación y recuperación de objetos (cuando se seleccionan los comportamientos de incorporación Balanced o Strict) o podría provocar atascos en las colas de ILM (cuando se selecciona el comportamiento de incorporación Dual commit). Consulta link:what-ilm-rule-is.html#ilm-rule-ingest-behavior["Comportamiento de procesamiento de reglas de ILM"].
