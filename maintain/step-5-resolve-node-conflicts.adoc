...
summary: 'En el paso 5 (resolver conflictos de nodos) del asistente para sitio de retirada, puede determinar si alguno de los nodos del sistema StorageGRID está desconectado o si alguno de los nodos del sitio seleccionado pertenece a un grupo de alta disponibilidad (ha). Después de resolver cualquier conflicto de nodo, se inicia el procedimiento de retirada desde esta página.' 
---
= Resuelve conflictos de nodos e inicia la descomisión de sitios en StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Todos los nodos de tu sistema StorageGRID deben estar conectados (image:icon_alert_green_checkmark.png["Icono Alerta verde marca de verificación"]).
+
...
.Pasos
. Consulta la sección sobre nodos desconectados del Paso 5 (Resolver conflictos entre nodos) para determinar si algún nodo de tu sistema StorageGRID tiene un estado de conexión Desconocido (image:icon_alarm_blue_unknown.png["Icono alarma azul desconocida"]) o Administrativamente inactivo (image:icon_alarm_gray_administratively_down.png["Icono alarma gris administrativamente abajo"]).
+
image:decommission_site_step_5_disconnected_nodes.png["Retirar del servicio Paso 5 resolver conflictos de nodos"]
...
+
image:decommission_site_step_5_ha_groups.png["Retirada de grupos de alta disponibilidad del paso 5 del sitio"]
...
+
Si todos los nodos están conectados y ningún nodo del sitio seleccionado forma parte de un grupo de HA, el campo *Contraseña de aprovisionamiento* se habilita.
...
+
image:decommission_site_step_5_provision_passphrase.png["Retirada de la frase de paso 5 de la instalación"]
...
+
image:decommission_site_step_5_warning.png["Advertencia de retirada del emplazamiento, paso 5"]
...
+
image:decommission_site_step_5_grid_configuration.png["Retirar la configuración de la cuadrícula del paso 5 del sitio"]
