...
---
= Configura un nodo de administración primario de reemplazo en StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
. Selecciona *Recuperar un nodo de administración principal que ha fallado*.
+
image:install_or_recover_primary_admin_node.png["Instale o recupere el nodo de administrador principal"]

. Cargue la copia de seguridad más reciente del paquete de recuperación:
+
.. Seleccione *examinar*.
.. Busca el archivo del paquete de recuperación más reciente para tu sistema StorageGRID y selecciona *Abrir*.
...
NOTE: Para configurar una relación de confianza con una parte confiable, consulta cómo configurar el inicio de sesión único usando link:../admin/learn-about-setting-up-sso-oidc.html["OIDC"] o link:../admin/learn-about-setting-up-sso-saml.html["SAML"]. Para acceder al certificado predeterminado del servidor, inicia sesión en el intérprete de comandos del nodo de administración. Ve al directorio `/var/local/mgmt-api` y selecciona el archivo `server.crt`.
