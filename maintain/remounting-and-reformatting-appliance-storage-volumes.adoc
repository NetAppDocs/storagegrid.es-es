...
summary: Si el procedimiento automático no consigue volver a montar los volúmenes de almacenamiento conservados ni reformatear los volúmenes de almacenamiento con errores, debes ejecutar manualmente dos scripts para completar esas operaciones. El primer script vuelve a montar los volúmenes que están correctamente formateados como volúmenes de almacenamiento de StorageGRID. El segundo script reformatea cualquier volumen desmontado, reconstruye la base de datos de Cassandra si es necesario e inicia los servicios. 
---
...
[role="lead"]
Si el procedimiento automático no consigue volver a montar los volúmenes de almacenamiento conservados ni reformatear los volúmenes de almacenamiento con errores, debes ejecutar manualmente dos scripts para completar esas operaciones. El primer script vuelve a montar los volúmenes que están correctamente formateados como volúmenes de almacenamiento de StorageGRID. El segundo script reformatea cualquier volumen desmontado, reconstruye la base de datos de Cassandra si es necesario e inicia los servicios.

Durante la recuperación de un nodo de almacenamiento, si aparece el mensaje «Esperando pasos manuales» en Grid Manager, sigue estos pasos para volver a montar y formatear los volúmenes de almacenamiento. Si no ves este mensaje, las operaciones de remontaje y formateo se han completado automáticamente, así que puedes omitir _este_ procedimiento y link:../maintain/restoring-object-data-to-storage-volume-for-appliance.html["restaurar datos de objeto"].
...
+
image:recovering_cassandra.png["Captura de pantalla que muestra el progreso de la recuperación en la interfaz de gestión de grid"]
...
+
El script te pregunta si deseas utilizar el proceso de restauración de volúmenes de Grid Manager para restaurar los datos de los objetos.

+
** En la mayoría de los casos, debes link:../maintain/restoring-object-data-to-storage-volume-for-appliance.html["Restaurar datos de objetos con Grid Manager"]. Responde `y` para usar Grid Manager.
** En casos excepcionales, como cuando sabes que el nodo de sustitución tiene menos volúmenes disponibles para el almacenamiento de objetos que el nodo original, ponte en contacto con soporte técnico para restaurar los datos de objetos manualmente. Si se da alguno de estos casos, responde `n` y pulsa cualquier tecla para volver a la línea de comandos.
