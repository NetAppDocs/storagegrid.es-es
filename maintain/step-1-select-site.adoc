...
---
= Selecciona un sitio para decomisionar en StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
image:decommission_site_step_select_site.png["Retirar sitio Paso 1 Seleccionar sitio"]
...
| Razón posible de retirada | Descripción | Paso siguiente 
 ...
 a| 
Marca de verificación verde (image:icon_alert_green_checkmark.png["Icono Alerta verde marca de verificación"])
 a| 
...
No. Todos los nodos de este sitio están desconectados. Póngase en contacto con el representante de cuenta de NetApp.
 a| 
No puedes decomisionar un sitio conectado a menos que todos los nodos del sitio estén conectados (image:icon_alert_green_checkmark.png["Icono Alerta verde marca de verificación"]).
 a| 
...
|===
+
El ejemplo muestra un sistema StorageGRID con tres sitios. La marca de verificación verde (image:icon_alert_green_checkmark.png["Icono Alerta verde marca de verificación"]) junto a los sitios Raleigh y Sunnyvale indica que puedes decomisionar esos sitios. Sin embargo, no puedes decomisionar el sitio Vancouver porque contiene el nodo de administración principal.
