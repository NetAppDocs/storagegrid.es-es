...
Archivo de paquete de recuperación `.zip`
 a| 
Obtenga una copia del archivo de Recovery Package más reciente `.zip`:
`sgws-recovery-package-_id-revision_.zip`

El contenido del `.zip` archivo se actualiza cada vez que se modifica el sistema. Se le indica que guarde la versión más reciente del paquete de recuperación en una ubicación segura después de realizar dichos cambios. Utilice la copia más reciente para recuperarse de fallos de la cuadrícula.
