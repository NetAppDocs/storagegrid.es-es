...
---
= Cambiar las direcciones IP de todos los nodos en una grid de StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
NOTE: Si el inicio de sesión único (SSO) está habilitado para tu sistema StorageGRID y se configuraron relaciones de confianza de terceros usando direcciones IP de nodos de administración (en vez de nombres de dominio completos, como se recomienda), prepárate para actualizar o reconfigurar estas relaciones de confianza de terceros en Active Directory Federation Services (AD FS) justo después de cambiar las direcciones IP. Consulta cómo configurar el SSO usando link:../admin/learn-about-setting-up-sso-oidc.html["OIDC"] o link:../admin/learn-about-setting-up-sso-saml.html["SAML"].
