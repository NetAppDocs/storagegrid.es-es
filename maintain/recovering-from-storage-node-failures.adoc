...
Póngase en contacto con el soporte técnico.
 a| 
Si han fallado varios nodos de almacenamiento, el soporte técnico debe colaborar en la recuperación para evitar inconsistencias en la base de datos de Cassandra que podrían provocar la pérdida de datos. Es posible que sea necesario aplicar un procedimiento de recuperación del sitio.

El soporte técnico puede determinar cuándo es seguro iniciar la recuperación de un segundo Storage Node.

*Nota*: Si en un sitio fallan varios nodos de almacenamiento que contienen el servicio ADC, se perderán todas las solicitudes de servicio de la plataforma pendientes para ese sitio.
...
Se produjo un error en uno o más volúmenes de almacenamiento, pero la unidad del sistema está intacta
 a| 
link:reviewing-warnings-about-storage-volume-recovery.html["Recupérese de un fallo en el volumen de almacenamiento, donde la unidad del sistema está intacta"]
 a| 
