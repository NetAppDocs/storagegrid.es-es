...
* Conoces la dirección IP de uno de los siguientes nodos de administración:
+
** El nodo de administración principal de la StorageGRID grid
** Un nodo de administración no principal, si está disponible
...
* Debes indicar la dirección IP del nodo de administración principal. Si el nodo de administración principal no está disponible, debes indicar la dirección IP del nodo de administración no principal, si está disponible. También debes confirmar el nombre de host (nombre del sistema) del nodo.
...
. En el campo *Nombre de nodo*, introduce el mismo nombre de host (nombre del sistema) que se utilizó para el nodo que estás recuperando y selecciona *Guardar*.
. En la sección «Instalación», comprueba que el estado actual sea «Listo para iniciar la instalación de `_node name_` en la grid con el Primary Admin Node `_admin_ip_`» y que el botón *Iniciar instalación* esté activado.
+
...
NOTE: Si necesitas acceder manualmente a la página de instalación de Monitor, selecciona *Monitor installation* en la barra de menús. Consulta https://docs.netapp.com/us-en/storagegrid-appliances/installconfig/monitoring-appliance-installation.html["Supervise la instalación del dispositivo"^].
