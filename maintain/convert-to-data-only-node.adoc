...
---
= Convertir un nodo de almacenamiento StorageGRID en un nodo solo de datos
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* Si el nodo de almacenamiento contiene el servicio ADC, hay más de dos nodos de almacenamiento ADC en línea en toda la grid y hay nodos de almacenamiento que no son ADC disponibles en el mismo sitio para alojar el servicio ADC.
...
Utiliza este procedimiento para convertir uno o varios nodos de almacenamiento combinados en nodos de solo datos. Si un nodo de almacenamiento contiene el servicio ADC, el servicio se traslada automáticamente a otro nodo de almacenamiento en el mismo sitio durante la conversión.
...
NOTE: StorageGRID traslada el servicio ADC a los nodos de almacenamiento más recientes que cumplen los requisitos para alojar dicho servicio.
