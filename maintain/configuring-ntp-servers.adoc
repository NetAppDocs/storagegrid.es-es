...
summary: 'Puede añadir, actualizar o quitar servidores de protocolo de tiempo de redes (NTP) para garantizar que los datos se sincronicen de forma precisa entre los nodos de grid del sistema StorageGRID.' 
---
= Administra los servidores NTP en StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
NOTE: Al especificar la fuente NTP externa para una instalación de StorageGRID en entorno de producción, no utilices el servicio Windows Time (W32Time) en versiones de Windows anteriores a Windows Server 2016. El servicio de hora en versiones anteriores de Windows no es lo suficientemente preciso y no está soportado por Microsoft para su uso en entornos de alta precisión, incluido StorageGRID. Para más detalles, consulta https://learn.microsoft.com/en-us/troubleshoot/windows-server/active-directory/support-boundary-high-accuracy-time["Límite de soporte para alta precisión de tiempo"^].
