...
summary: Puedes cambiar la configuración de red de uno o varios nodos usando la herramienta Change IP. Puedes cambiar la configuración de la Grid Network, o añadir, cambiar o eliminar las Admin o Client Networks. También puedes cambiar la configuración de IP de la LAN para un nodo BMC de un dispositivo. 
---
= Cambiar la configuración de la red de nodos en StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Puedes cambiar la configuración de red de uno o varios nodos usando la herramienta Change IP. Puedes cambiar la configuración de la Grid Network, o añadir, cambiar o eliminar las Admin o Client Networks. También puedes cambiar la configuración de IP de la LAN para un nodo BMC de un dispositivo.
...
NOTE: Si el inicio de sesión único (SSO) está habilitado en tu sistema StorageGRID y vas a cambiar la dirección IP de un nodo de administración, ten en cuenta que cualquier relación de confianza de parte confiable que se haya configurado usando la dirección IP del nodo de administración (en lugar de su dominio completo, como se recomienda) dejará de ser válida. Ya no podrás iniciar sesión en el nodo. Justo después de cambiar la dirección IP, debes actualizar o reconfigurar la relación de confianza de la parte confiable del nodo en Active Directory Federation Services (AD FS) con la nueva dirección IP. Consulta las instrucciones para configurar el SSO usando link:../admin/learn-about-setting-up-sso-oidc.html["OIDC"] o link:../admin/learn-about-setting-up-sso-saml.html["SAML"].
...
+
image:change_ip_tool_main_menu.png["Captura de pantalla que muestra la pantalla de bienvenida de la herramienta Change IP"]
...
+
image:change_ip_tool_edit_ip_mask_sample_output.png["captura de pantalla descrita por el texto circundante"]
...
+
image:change_ip_tool_validate_sample_error_messages.gif["captura de pantalla descrita por el texto circundante"]

+
En este ejemplo, se ha aprobado la validación.

+
image:change_ip_tool_validate_sample_passed_messages.gif["captura de pantalla descrita por el texto circundante"]
...
== Cambiar la configuración de la dirección IP LAN BMC del nodo del dispositivo

Puedes cambiar la configuración de la dirección IP LAN de un nodo BMC del dispositivo.

. En Grid Manager, ve a *Nodos* > *_grid node_* > *BMC settings*.
. En la sección de configuración de IP de la LAN, selecciona un tipo de asignación de IP *Estática* o *DHCP*.
. Introduce la dirección MAC, la dirección IPv4 (CIDR) y la puerta de enlace predeterminada para el BMC.
. Seleccione *Guardar*.


También puedes modificar o consultar la configuración de la dirección IP de la LAN de BMC usando la `PUT /bmc-configuration/root-password` o la `GET /bmc-configuration/root-password` API.
