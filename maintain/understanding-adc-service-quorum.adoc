...
summary: 'El quórum de ADC garantiza que un número mínimo de servicios de controlador de dominio administrativo (ADC) permanezcan disponibles en cada sitio. StorageGRID mueve automáticamente el servicio ADC durante el proceso de decomisionar cuando es necesario.' 
---
= ¿Qué es el quórum de StorageGRID ADC?
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
El quórum de ADC garantiza que un número mínimo de servicios de controlador de dominio administrativo (ADC) permanezcan disponibles en cada sitio. StorageGRID mueve automáticamente el servicio ADC durante el proceso de decomisionar cuando es necesario.
...
Cuando decomisionas un nodo de almacenamiento que tiene el servicio ADC, StorageGRID mueve automáticamente el servicio ADC a otro nodo de almacenamiento en el mismo sitio si al decomisionar el nodo el número de ADC en el sitio quedaría por debajo del quórum. El traslado automático requiere que todos los nodos de la grid estén conectados y que haya nodos de almacenamiento que no sean ADC disponibles en el mismo sitio para recibir el servicio. El procedimiento de decomisionar mantiene un mínimo de tres nodos de almacenamiento ADC por sitio. Por ejemplo, si decomisionas cuatro o seis nodos ADC en un sitio, quedarán tres nodos de almacenamiento ADC después del procedimiento, siempre que existan nodos no ADC disponibles en el mismo sitio.

Si no es posible trasladar automáticamente el servicio ADC, puedes link:move-adc-service.html["mueve manualmente el servicio ADC"] a otro nodo de almacenamiento en el mismo sitio o link:../expand/index.html["agrega un nuevo Storage Node en una expansión"] y especificar que debe tener un servicio ADC. Luego, puedes decomisionar el nodo de almacenamiento existente sin afectar el quórum.
