...
---
= Consideraciones para decomisionar un sitio en StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
NOTE: Si un sitio contiene una combinación de nodos conectados (image:icon_alert_green_checkmark.png["Icono Alerta verde marca de verificación"]) y nodos desconectados (image:icon_alarm_gray_administratively_down.png["Icono alarma gris administrativamente abajo"] o image:icon_alarm_blue_unknown.png["Icono alarma azul desconocida"]), debes volver a poner en línea todos los nodos que estén fuera de línea.
...
* No puedes decomisionar un sitio si contiene una mezcla de nodos conectados (image:icon_alert_green_checkmark.png["Icono Alerta verde marca de verificación"]) y desconectados (image:icon_alarm_blue_unknown.png["Icono alarma azul desconocida"] o image:icon_alarm_gray_administratively_down.png["Icono alarma gris administrativamente abajo"]).
* No puedes decomisionar un sitio si algún nodo de cualquier otro sitio está desconectado (image:icon_alarm_blue_unknown.png["Icono alarma azul desconocida"] o image:icon_alarm_gray_administratively_down.png["Icono alarma gris administrativamente abajo"]).
...
* Todos los nodos de tu sistema StorageGRID deben tener un estado de conexión *Conectado* (image:icon_alert_green_checkmark.png["Icono Alerta verde marca de verificación"]); sin embargo, los nodos pueden tener alertas activas.
+
...
+
** *Desconocido* (image:icon_alarm_blue_unknown.png["Icono alarma azul desconocida"]): Por una razón desconocida, un nodo se ha desconectado o los servicios del nodo han dejado de funcionar de forma inesperada. Por ejemplo, es posible que se haya detenido un servicio del nodo o que el nodo haya perdido la conexión de red debido a un corte de energía o a una interrupción inesperada.
** *Fuera de servicio por motivos administrativos* (image:icon_alarm_gray_administratively_down.png["Icono alarma gris administrativamente abajo"]): El nodo no está conectado a la grid por una razón prevista. Por ejemplo, el nodo o los servicios en el nodo se han apagado de forma controlada.


* Todos los nodos de los demás sitios deben tener un estado de conexión *Conectado* (image:icon_alert_green_checkmark.png["Icono Alerta verde marca de verificación"]); sin embargo, estos otros nodos pueden tener alertas activas.
