...
---
= Volver a montar y formatear los volúmenes de almacenamiento StorageGRID (pasos manuales)
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Durante la recuperación de un nodo de almacenamiento, si aparece el mensaje «Esperando pasos manuales» en Grid Manager, sigue estos pasos para volver a montar y formatear los volúmenes de almacenamiento. Si no ves este mensaje, las operaciones de montaje y formateo se han completado automáticamente, así que puedes omitir _este_ procedimiento y link:../maintain/restoring-object-data-to-storage-volume.html["restaurar datos de objeto"].
...
+
** En la mayoría de los casos, debes link:../maintain/restoring-object-data-to-storage-volume.html["Restaurar datos de objetos con Grid Manager"]. Responde `y` para usar Grid Manager.
