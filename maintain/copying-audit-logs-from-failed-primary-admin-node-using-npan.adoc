...
---
= Copiar los registros de auditoría de StorageGRID desde un nodo de administración principal que ha fallado
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.Pasos
. Inicie sesión en el nodo de administración principal que falló, si es posible.  De lo contrario, inicie sesión en cualquier nodo de administración disponible.
+
.. Introduzca el siguiente comando: `ssh admin@_grid_node_IP_`
.. Introduzca la contraseña que aparece en el `Passwords.txt` archivo.
.. Introduce el siguiente comando para cambiar a la cuenta de usuario raíz: `su -`
.. Introduzca la contraseña que aparece en el `Passwords.txt` archivo.
+
...
+
(o `cd /var/local/log` en sistemas anteriores)

. Cambie el nombre del archivo de registro de origen a un nombre de archivo numerado único.
+
[NOTE]
====
** Si el archivo de registro está en el `/var/local/audit/export` directorio, su nombre es `audit.log` .
** Si el archivo de registro está en el `/var/local/log` directorio, su nombre es `localaudit.log` .


====
+
Por ejemplo, cambie el nombre del `localaudit.log` archivo a `2025-10-22.txt.1` .
...
+
Cuando se te solicite, introduce la contraseña de la cuenta de usuario admin.
...
. Cerrar sesión como usuario raíz: `exit`.
