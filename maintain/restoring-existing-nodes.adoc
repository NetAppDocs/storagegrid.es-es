...
Si recuperas un nodo desde un nodo de administración no principal, configura ADMIN_IP con la dirección IP de la red de la malla del _nodo de administración no principal_, de modo que el nodo recuperado se registre en el nodo de administración no principal en lugar de en el nodo de administración principal. Para consultar las reglas para las claves de la red de administración, consulta link:../swnodes/creating-node-configuration-files.html["Crea archivos de configuración de nodos StorageGRID para implementaciones en Linux"].
