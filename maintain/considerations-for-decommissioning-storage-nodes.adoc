...
---
= Consideraciones para decomisionar un Storage Node en StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* El sistema debe, en todo momento, incluir suficientes nodos de almacenamiento para satisfacer los requisitos operativos, incluidos el link:understanding-adc-service-quorum.html["Quórum ADC"] y el link:reviewing-ilm-policy-and-storage-configuration.html["Política de ILM"] activos. Si un proceso de decomisionar hace que el recuento de ADC de un sitio quede por debajo del quórum, StorageGRID mueve automáticamente el servicio ADC a nodos de almacenamiento disponibles que no sean ADC en el mismo sitio, manteniendo el quórum de ADC. Si el servicio ADC no se puede mover automáticamente, puede que necesites link:../expand/adding-grid-nodes-to-existing-site-or-adding-new-site.html["agrega un nuevo nodo de almacenamiento en una operación de expansión"] antes de poder decomisionar un nodo de almacenamiento existente.
+
