...
---
= Apaga y enciende todos los nodos StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Cuando apagas todos los nodos de un sitio o grid, no puedes acceder a los objetos ingeridos mientras los Storage Nodes estén desconectados.
...
Detén primero los servicios en los nodos de administración y los nodos de puerta de enlace, luego detén los servicios en los nodos de almacenamiento.
...
NOTE: Si un host incluye varios nodos de grid, los nodos vuelven a estar en línea automáticamente cuando enciendes el host.

.Pasos
. Encienda los hosts del nodo de administrador principal y los nodos de administrador que no son primarios.
+

NOTE: No podrás iniciar sesión en los nodos de administración hasta que se hayan reiniciado los nodos de almacenamiento.
