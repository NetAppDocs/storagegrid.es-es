...
Servicio ADC (nodos de almacenamiento combinados o solo metadatos)
 a| 
Si este nodo de almacenamiento ejecutará el servicio de controlador de dominio administrativo (ADC). El servicio ADC realiza un seguimiento de la ubicación y disponibilidad de los servicios de red. Al menos tres nodos de almacenamiento en cada sitio deben incluir el servicio ADC.

*** Seleccione *Sí* si el nodo de almacenamiento que va a reemplazar incluye el servicio ADC. Debido a que no puede retirar un nodo de almacenamiento si quedan muy pocos servicios ADC, esta configuración garantiza que un nuevo servicio ADC esté disponible antes de eliminar el servicio antiguo.
*** Puede link:../maintain/move-adc-service.html["Mueva el servicio ADC a otro nodo de almacenamiento del mismo sitio"]asegurarse de que se cumple el quórum de servicio ADC.
...
+
Debe descargar una copia actualizada del archivo del paquete de recuperación lo antes posible después de realizar cambios de topología de grid en el sistema StorageGRID. El archivo de paquete de recuperación le permite restaurar el sistema en caso de que se produzca un fallo.
