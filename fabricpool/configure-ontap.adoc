...
Estilo de URL
 a| 
Si link:../admin/configuring-s3-api-endpoint-domain-names.html["Nombres de dominio de punto final S3 configurados"] seleccionas *URL de alojamiento virtual*.

*URL de tipo ruta* es la opción predeterminada para ONTAP, pero se recomienda utilizar solicitudes de tipo «host virtual» para StorageGRID. Debes utilizar *URL de tipo ruta* si introduces una dirección IP en lugar de un nombre de dominio en el campo *Server name (FQDN)*.
