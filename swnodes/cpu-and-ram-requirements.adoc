...
---
= Requisitos de CPU y RAM para StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
** Si hay varios inquilinos y cada uno tiene hasta 5,000 buckets, al menos 64 GB por nodo
+

NOTE: El sistema StorageGRID admite un máximo de 20,000 buckets por cada cuenta de inquilino S3.
...
CAUTION: Supervisa periódicamente el uso de la CPU y la memoria para asegurarte de que estos recursos sigan siendo suficientes para tu carga de trabajo. Por ejemplo, duplicar la asignación de RAM y CPU para los nodos de almacenamiento virtual proporcionaría recursos similares a los que se ofrecen para los nodos de los dispositivos StorageGRID. Además, si la cantidad de metadatos por nodo supera los 500 GB, considera aumentar la RAM por nodo a 48 GB o más. Para obtener información sobre la gestión del almacenamiento de metadatos de objetos, aumentar el ajuste de Espacio reservado para metadatos y supervisar el uso de la CPU y la memoria, consulta las instrucciones de link:../admin/index.html["administración"], link:../monitor/index.html["Supervisión"] y link:../upgrade/index.html["actualizando"] StorageGRID.
...
Consulta link:storage-and-performance-requirements.html["Los requisitos de almacenamiento y rendimiento"].
