...
---
= Aprueba los nodos de grid pendientes para StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
image:5_gmi_installer_grid_nodes_pending.gif["Esta imagen se explica por el texto circundante."]
...
.. Vuelve a la página de inicio y selecciona *Iniciar instalación*.
...
+
image:7_gmi_installer_grid_nodes_approved.gif["Esta imagen se explica por el texto circundante."]

. Repita estos pasos para cada nodo de cuadrícula pendiente que desee aprobar.
+
Debes aprobar todos los nodos que quieras en la grid. Sin embargo, puedes volver a esta página en cualquier momento antes de seleccionar *Install* en la página Summary. Puedes modificar las propiedades de un nodo de grid aprobado seleccionando su botón de opción y seleccionando *Edit*.

. Cuando hayas terminado de aprobar los nodos de la red, selecciona *Siguiente*.
