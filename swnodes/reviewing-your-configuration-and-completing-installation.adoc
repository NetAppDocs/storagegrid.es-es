...
---
= Revisa tu configuración de StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
image:11_gmi_installer_summary_page.gif["Página de resumen del instalador de Grid Manager"]
...
NOTE: Si un nodo está configurado para utilizar la red del cliente, la puerta de enlace predeterminada de ese nodo cambia de la red de grid a la red del cliente cuando seleccionas *Instalar*. Si pierdes la conectividad, debes asegurarte de que estás accediendo al nodo de administración principal a través de una subred accesible. Consulta link:../network/index.html["Directrices sobre redes"] para más detalles.
...
. Marca la casilla *He descargado y comprobado correctamente el archivo del paquete de recuperación* y selecciona *Siguiente*.
+
...
+
image:12_gmi_installer_status_page.gif["Esta imagen se explica por el texto circundante."]
