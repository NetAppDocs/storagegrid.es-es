...
---
= Requisitos de almacenamiento y rendimiento para StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Debes conocer los requisitos de almacenamiento de los nodos de StorageGRID para asegurarte de que tienes suficiente espacio para la configuración inicial y la expansión futura de almacenamiento.
...
* *Pool de contenedores* -- almacenamiento de alto rendimiento (10K SAS o SSD) para los contenedores de los nodos, que se asigna al controlador de almacenamiento del motor de contenedores al instalar y configurar el motor de contenedores en los hosts que están soportando tus nodos de StorageGRID.
* *Datos del sistema* -- almacenamiento de alto rendimiento (SAS de 10K o SSD) para el almacenamiento persistente por nodo de los datos del sistema y los registros de transacciones, que los servicios del host de StorageGRID consumen y asignan a los nodos individuales.
...
Debes utilizar dispositivos de bloques respaldados por RAID para todas las categorías de almacenamiento. No se admiten discos no redundantes, SSD ni JBOD. Puedes utilizar almacenamiento RAID compartido o local para cualquier categoría de almacenamiento. Para migrar nodos en StorageGRID, debes almacenar tanto los datos del sistema como los datos de los objetos en almacenamiento compartido. Para obtener más información, consulta link:node-container-migration-requirements.html["Requisitos de migración de contenedores de nodos"].
...
NOTE: En una implementación de producción, no ejecutes más de un nodo de almacenamiento en un solo host físico o virtual. Un host dedicado para cada nodo de almacenamiento proporciona un dominio de fallo aislado.

Puedes implementar otros tipos de nodos, como nodos de administración o nodos de puerta de enlace, en los mismos hosts, o puedes implementarlos en sus propios hosts dedicados, según sea necesario.


NOTE: No puedes usar instantáneas de disco para restaurar nodos de la grid. En su lugar, consulta los procedimientos de link:../maintain/warnings-and-considerations-for-grid-node-recovery.html["recuperación de nodo de grid"] para cada tipo de nodo.
...
La siguiente tabla muestra el número de volúmenes de almacenamiento (LUN) necesarios para cada host y el tamaño mínimo requerido para cada LUN, en función de los nodos que se hayan implementado en dicho host.

El tamaño máximo de LUN probado es de 100 TiB.
...
*Nota:* Un nodo de almacenamiento basado en software de Linux y un nodo de almacenamiento basado en software de VMware pueden tener entre 1 y 48 volúmenes de almacenamiento. Se recomienda al menos 3 volúmenes de almacenamiento.
 a| 
12 TB (4 TB/LUN, mínimo)

Tamaño máximo de LUN probado: 100 TiB.

Consulta <<storage_req_SN,Requisitos de almacenamiento para nodos de almacenamiento>> para más información.
...
NOTE: En función del nivel de auditoría configurado, el tamaño de las entradas del usuario como el nombre de la clave de un objeto S3 y la cantidad de datos de registro de auditoría que necesites conservar, puede que tengas que aumentar el tamaño del LUN del registro de auditoría en cada Admin Node. Por lo general, una grid genera aproximadamente 1 KB de datos de auditoría por cada operación de S3, lo que significa que un LUN de 200 GB admitiría 70 millones de operaciones al día o 800 operaciones por segundo durante dos o tres días.
...
La siguiente tabla muestra el espacio de almacenamiento mínimo necesario para cada tipo de nodo. Puedes usar esta tabla para determinar la cantidad mínima de almacenamiento que debes proporcionar al host en cada categoría de almacenamiento, según los nodos que estén implementados en ese host.
...
Supongamos que planeas implementar tres nodos en el mismo host o máquina virtual: un Storage Node, un Admin Node y un Gateway Node. Deberías proporcionar un mínimo de nueve volúmenes de almacenamiento al host. Necesitas un mínimo de 300 GB de almacenamiento de nivel de rendimiento para los contenedores de los nodos, 700 GB de almacenamiento de nivel de rendimiento para los datos del sistema y los registros de transacciones, y 12 TB de almacenamiento de nivel de capacidad para los datos de objetos.
...
Linux y VMware tienen los siguientes requisitos de almacenamiento para los nodos de almacenamiento:

* Un nodo de almacenamiento basado en software Linux puede tener entre 1 y 48 volúmenes de almacenamiento.
* Un nodo de almacenamiento basado en software de VMware puede tener de 1 a 48 volúmenes de almacenamiento.
...
image:metadata_space_storage_node.png["Nodo de almacenamiento de espacio de metadatos"]
...
Al instalar una grid con nodos de almacenamiento exclusivos de metadatos, la grid también debe contener un número mínimo de nodos para el almacenamiento de objetos. Consulta link:../primer/what-storage-node-is.html#types-of-storage-nodes["Tipos de nodos de almacenamiento"] para más información sobre los nodos de almacenamiento exclusivos de metadatos.
...
* Al configurar diferentes nodos de almacenamiento para un sitio, utiliza la misma configuración para el volumen 0 siempre que sea posible. Si un sitio contiene nodos de almacenamiento de diferentes tamaños, el nodo de almacenamiento con el volumen 0 más pequeño determina la capacidad de metadatos de ese sitio.
