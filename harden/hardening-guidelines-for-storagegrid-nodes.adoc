...
Puedes activar o desactivar el acceso IPMI remoto para todos los dispositivos que incluyan un BMC. La interfaz IPMI remota permite el acceso a bajo nivel de hardware a tus dispositivos StorageGRID a cualquier persona que tenga una cuenta y contraseña de BMC. Si no necesitas acceso IPMI remoto al BMC, desactiva esta opción.

* Para controlar el acceso IPMI remoto al BMC en Grid Manager, ve a *Configuración* > *Seguridad* > *Ajustes de seguridad* > *Dispositivos* > *Acceso IPMI remoto*:
+
...
== Cambiar la contraseña BMC del dispositivo

Puedes cambiar la contraseña de BMC de un dispositivo para limitar el acceso y mejorar la seguridad del sistema.

. En Grid Manager, ve a *Nodos* > *_grid node_* > *BMC settings*.
. En la sección «Cambiar contraseña», introduce una nueva contraseña de BMC de entre 8 y 16 caracteres en *Nueva contraseña* y luego vuelve a introducirla en *Confirmar nueva contraseña*.
. Seleccione *Guardar*.


También puedes cambiar la contraseña del BMC usando la  `PUT /bmc-configuration/root-password` API.



[[control-remote-access-through-usb-portscontrol-external-usb-port-access-on-appliances]]
== [[control-remote-access-through-usb-ports]]Controla el acceso a los puertos USB externos en los dispositivos

Puedes habilitar los puertos USB de los dispositivos para permitir el acceso del soporte técnico con fines de diagnóstico y resolución de problemas. Si no necesitas acceso a los puertos USB, desactiva los puertos por seguridad. Los puertos USB están habilitados de forma predeterminada después de la instalación inicial o una actualización.

* En el Grid Manager, ve a *Configuración* > *Seguridad* > *Ajustes de seguridad* > *Dispositivos* > *Acceso a los puertos USB* para activar o desactivar los puertos USB:
+
** Marca la casilla *Habilitar puertos USB* y guarda para habilitar los puertos.
+
Si la casilla de selección USB estaba desmarcada y se había guardado, marca la casilla *Habilitar puertos USB* y guarda para volver a habilitar los puertos USB.

** Desmarca la casilla *Habilitar puertos USB* y guarda para desactivar los puertos.
...
* No permitas que los clientes no fiables se conecten directamente a los nodos de almacenamiento. Utiliza un endpoint de equilibrador de carga servido por un Gateway Node o un equilibrador de carga de terceros.
