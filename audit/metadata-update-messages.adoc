...
---
= Mensajes de auditoría para actualizaciones de metadatos de objetos S3 en StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
El cliente S3 realiza una solicitud (SUPD) para actualizar los metadatos especificados (`x-amz-meta-*`) del objeto S3 (S3KY).

En este ejemplo, los encabezados de solicitud se incluyen en el campo HTRH, ya que se ha configurado como encabezado del protocolo de auditoría (*Configuración* > *Supervisión* > *Servidor de auditoría y syslog*). Véase link:../monitor/configure-log-management.html["Configurar la gestión de registros y el servidor syslog externo"].
