...
summary: Cuando un cliente S3 emite una operación HEAD, se realiza una solicitud para verificar la existencia de un objeto o depósito y recuperar los metadatos sobre un objeto.  El servidor emite este mensaje si la operación es exitosa. 
---
= Mensaje de auditoría SHEA para operaciones S3 HEAD en StorageGRID
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
ASRS
 a| 
URN de la sesión del rol asumido
 a| 
El identificador único de la sesión del rol asumido.

Solo aparece cuando la solicitud S3 utiliza credenciales de seguridad temporales procedentes de una solicitud de AssumeRole.
...
URN del rol asumido
 a| 
El identificador único del rol asumido.
